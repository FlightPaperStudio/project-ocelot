...
namespace ProjectOcelot.Match
{
	public static class MatchSettings
	{
		...
		/// </summary>
		public static Match.Setup.Debate MatchDebate
		{
			get;
			private set;
		}
		...
		public static void SetMatchSettings ( MatchType type, bool turnTimer = true, float timePerTurn = 90f, int heroesPerTeam = 3, bool heroLimit = true, List<UnitSettingData> heroes = null )
		{
			...
			MatchDebate = Match.Setup.DebateGenerator.GetRandomDebate ( Type );
		...
		{
			// Check match type
			if ( Type == MatchType.Classic || Type == MatchType.CustomClassic || Type == MatchType.Inferno || Type == MatchType.CustomInferno )
			{
				if ( turnOrder == 1 )
			...
			else if ( Type == MatchType.Control || Type == MatchType.CustomControl )
			{
				if ( turnOrder == 1 )
					return Player.Direction.LEFT_TO_RIGHT;
				else if ( turnOrder == 2 )
