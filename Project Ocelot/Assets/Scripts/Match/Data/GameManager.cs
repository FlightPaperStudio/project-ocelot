...
namespace ProjectOcelot.Match
{
	public class GameManager : MonoBehaviour
	{
		...
		{
			public Units.Unit TargetUnit;
			public Player Owner;
			public TurnAnimation [ ] Animation;

			public PostTurnAnimation ( Units.Unit unit, Player owner, params TurnAnimation [ ] animation )
			{
				TargetUnit = unit;
				Owner = owner;
				Animation = animation;
		...
		[HideInInspector]
		public Units.Unit.KOdelegate GlobalKOdelegate = null;

		[HideInInspector]
		public List<Units.Unit> UnitQueue = new List<Units.Unit> ( );
		...
		/// </summary>
		public Units.Unit SelectedUnit
		{
			get;
			private set;
		}
		...
		private void StartMatch ( )
		{
			...
			for ( int i = 0; i < players.Length; i++ )
			{
				...
				for ( int j = 0; j < players [ i ].Units.Count; j++ )
				{
					...
					Units.Unit unit = Instantiate ( unitPrefabDictionary [ players [ i ].Units [ j ].ID ], players [ i ].transform );
					...
					// Set unit direction
					Tools.Util.OrientSpriteToDirection ( unit.sprite, players [ i ].TeamDirection );
				...
				if ( players [ i ].StandardKOdelegate != null )
					foreach ( Units.Unit unit in players [ i ].UnitInstances )
			...
			if ( GlobalKOdelegate != null )
				foreach ( Player player in players )
					foreach ( Units.Unit unit in player.UnitInstances )
		...
		public void GetTeamMoves ( )
		{
			...
			foreach ( Units.Unit unit in CurrentPlayer.UnitInstances )
			{
				// Set move list
		...
		private IEnumerator StartTurnCoroutine ( )
		{
			...
			yield return UI.splash.Slide ( "<size=80%>Round " + Round + "\n</size>" + CurrentPlayer.PlayerName + "'s Turn", Tools.Util.TeamColor ( CurrentPlayer.Team ), true ).WaitForCompletion ( );
		...
		private void UpdateUnitCountdowns ( )
		{
			...
			{
				// Set cooldowns
				if ( unit is Units.HeroUnit )
				{
					Units.HeroUnit hero = unit as Units.HeroUnit;
					hero.Cooldown ( );
				}
			}
		}
		...
		public void PreviewUnitMoves ( Units.Unit unit )
		{
		...
		public void SelectCommandTarget ( Hex hex )
		{
			...
			( SelectedUnit as Units.HeroUnit ).SelectCommandTile ( hex );
		}
		...
		public void ExecuteCommand ( )
		{
			...
			( SelectedUnit as Units.HeroUnit ).ExecuteCommand ( );
		...
		public void CancelCommand ( )
		{
			...
			( SelectedUnit as Units.HeroUnit ).CancelCommand ( );
		...
		private void DisplayAvailableMoves ( Units.Unit unit, MoveData prerequisite )
		{
		...
		private bool ForfeitCheck ( )
		{
			...
			foreach ( Units.Unit u in CurrentPlayer.UnitInstances )
		...
		private bool WinnerCheck ( )
		{
			...
			foreach ( Player player in players )
			{
				Units.Unit leader = player.UnitInstances.Find ( x => x is Units.Leader );
		...
		/// </summary>
		private void BringUnitToTheFront ( Units.Unit u )
		{
