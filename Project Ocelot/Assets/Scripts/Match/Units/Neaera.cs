	...
	public class Neaera : Leader
	{
		...
		public override void ExecuteCommand ( )
		{
			...
			if ( GM.SelectedCommand.PrimaryTarget.Tile.CurrentUnit != null )
			{
				...
				// Apply shield
				GM.SelectedCommand.PrimaryTarget.Tile.CurrentUnit.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.PLASMA_SHIELD, InstanceData.Ability1.Duration, this );
			}
			...
			currentSolarFlare.Icon.color = Tools.Util.TeamColor ( Owner.Team );
		...
		private List<Tween> ApplyBlind ( Hex hex )
		{
			...
			for ( int i = 0; i < targets.Count; i++ )
			{
				...
				// Apply blind
				targets [ i ].Tile.CurrentUnit.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.BLIND, InstanceData.Ability1.Duration, this );
		...
		private void RemoveSolarFlare ( )
		{
			...
			if ( currentSolarFlare.CurrentHex.Tile.CurrentUnit != null )
				currentSolarFlare.CurrentHex.Tile.CurrentUnit.Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.PLASMA_SHIELD, this );
