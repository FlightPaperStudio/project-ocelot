	...
	public class MindControl : HeroUnit
	{
		...
		protected override void GetAssisted ( MoveData data )
		{
			...
			foreach ( Hex hex in data.AssistTargets )
			{
				...
				if ( InstanceData.Ability1.IsPerkEnabled && mindControlledUnits.Contains ( hex.Tile.CurrentUnit ) )
				{
					...
					hex.Tile.CurrentUnit.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.MIND_CONTROLLED, InstanceData.Ability1.Duration, this );
				}
			}
		}
		...
		public override void Cooldown ( )
		{
			...
			if ( InstanceData.Ability1.IsEnabled )
			{
				...
				List<Unit> expiredUnits = mindControlledUnits.FindAll ( x => !x.Status.HasStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.MIND_CONTROLLED ) );
				...
				for ( int i = 0; i < expiredUnits.Count; i++ )
				{
					...
					// Get original owner
					Match.Player player = GetOriginalOwner ( expiredUnits [ i ] );
		...
		protected override void UseSpecial ( MoveData data )
		{
			...
			Color32 c = Tools.Util.TeamColor ( Owner.Team );
		...
		{
			// Create animation
			Tween t = unit.sprite.DOColor ( Tools.Util.TeamColor ( Owner.Team ), MIND_CONTROL_ANIMATION_TIME )
				.OnStart ( ( ) =>
				{
				// Check for pre-existing Mind Control
				if ( unit.Status.Effects.Exists ( x => x.ID == (int) Database.StatusEffectDatabase.StatusEffectType.MIND_CONTROLLED ) )
					{
					// Remove pre-existing Mind Control
					MindControl original = unit.Status.Effects.Find ( x => x.ID == (int)Database.StatusEffectDatabase.StatusEffectType.MIND_CONTROLLED ).Caster as MindControl;
				...
				// Apply status effect
				unit.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.MIND_CONTROLLED, InstanceData.Ability1.Duration, this );
		...
		private void DeactivateMindControl ( Unit unit, bool usePostAnimationQueue = false )
		{
			...
			Match.Player player = GetOriginalOwner ( unit );

			// Create animation
			Tween t = unit.sprite.DOColor ( Tools.Util.TeamColor ( player.Team ), MIND_CONTROL_ANIMATION_TIME )
				.OnComplete ( ( ) =>
				{
				// Apply status effect
				unit.Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.MIND_CONTROLLED, this );
			...
			if ( usePostAnimationQueue )
				GM.PostAnimationQueue.Add ( new Match.GameManager.PostTurnAnimation ( unit, player, new Match.GameManager.TurnAnimation ( t, true ) ) );
			else
				GM.AnimationQueue.Add ( new Match.GameManager.TurnAnimation ( t, true ) );
		}
		...
		private Match.Player GetOriginalOwner ( Unit unit )
		{
			// Check each player
			foreach ( Match.Player player in GM.Players )
		...
		private void UnitTeamSwitch ( Unit unit, Match.Player player )
		{
			...
			Tools.Util.OrientSpriteToDirection ( unit.sprite, player.TeamDirection );
