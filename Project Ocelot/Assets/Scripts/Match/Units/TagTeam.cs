	...
	public class TagTeam : HeroUnit
	{
		...
		public override void InitializeInstance ( Match.GameManager gm, int instanceID, UnitSettingData settingData )
		{
			...
			if ( InstanceData.Ability1.IsEnabled && !isSplit )
				Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.HASTE, StatusEffects.PERMANENT_EFFECT, this );
			...
			duoData = Match.MatchSettings.GetUnitData ( ID );
			partner1Data = Match.MatchSettings.GetUnitData ( ID + 1 );
			partner2Data = Match.MatchSettings.GetUnitData ( ID + 2 );
		...
		private bool PartnerMoveCheck ( MoveData data )
		{
			...
			if ( !Status.HasStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.IN_UNISON ) || !tagTeamPartner.Status.HasStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.IN_UNISON ) )
				return false;
		...
		private void SplitUp ( Hex hex )
		{
			...
			Tween t = tagTeamPartner.transform.DOMove ( hex.transform.position, MOVE_ANIMATION_TIME )
				...
				Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.HASTE, this );
					tagTeamPartner.Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.HASTE, tagTeamPartner );

				// Apply status effect
				Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.IN_UNISON, InstanceData.Ability2.Duration, this );
					tagTeamPartner.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.IN_UNISON, InstanceData.Ability2.Duration, this );
		...
		private void Regroup ( Hex hex )
		{
			...
			Tween t = transform.DOMove ( hex.transform.position, MOVE_ANIMATION_TIME )
				...
				if ( originalPartner == this )
					...
					// Remove status effect
					Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.IN_UNISON, this );
					...
					// Remove status effect
					tagTeamPartner.Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.IN_UNISON, this );

					// Apply status effect
					tagTeamPartner.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.HASTE, StatusEffects.PERMANENT_EFFECT, this );
