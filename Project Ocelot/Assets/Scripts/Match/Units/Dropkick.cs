	...
	public class Dropkick : HeroUnit
	{
		...
		public override void ExecuteCommand ( )
		{
			...
			s.OnComplete ( ( ) =>
			{
				...
				// Apply status effect
				Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.EXHAUSTION, InstanceData.Ability2.Duration, this );
		...
		protected override void UseSpecial ( MoveData data )
		{
			...
			Tween t1 = transform.DOMove ( data.Destination.transform.position, DIVE_ANIMATION_TIME )
				...
				// Apply status effect
				Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.STUNNED, InstanceData.Ability1.Duration, this );
					GM.UI.matchInfoMenu.GetPlayerHUD ( this ).UpdateStatusEffects ( InstanceID, Status );
		...
		private void DivebombKnockback ( Unit target, Hex destination, bool isAppend )
		{
			...
			// Add animations to queue
			GM.AnimationQueue.Add ( new Match.GameManager.TurnAnimation ( t, isAppend ) );
		}
		...
		private Hex GetNextDropkickTarget ( Hex hex, int direction )
		{
			...
			{
				// Check if the unit is incorporeal
				if ( hex.Tile.CurrentUnit.Status.Effects.Exists ( x => x.ID == (int) Database.StatusEffectDatabase.StatusEffectType.INCORPOREAL ) )
