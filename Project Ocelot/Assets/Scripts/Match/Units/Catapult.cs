	...
	public class Catapult : HeroUnit
	{
		...
		public override void ExecuteCommand ( )
		{
			...
			Sequence s = DOTween.Sequence ( )
				...
				currentGrappleDisplay = Instantiate ( grappleDisplayPrefab, Owner.transform );
					...
					Tools.Util.OrientSpriteToDirection ( currentGrappleDisplay, Owner.TeamDirection );

				// Start cooldown
				StartCooldown ( InstanceData.Ability2 );

				// Apply hero's status effect
				Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.GRAPPLE_HOLD, InstanceData.Ability2.Duration, this );
					GM.UI.matchInfoMenu.GetPlayerHUD ( this ).UpdateStatusEffects ( InstanceID, Status );

				// Apply target's status effect
				grappleTarget.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.GRAPPLED, InstanceData.Ability2.Duration + 1, this );
		...
		public override void InteruptUnit ( )
		{
			...
			if ( grappleTarget != null )
			{
				...
				Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.GRAPPLE_HOLD, this );
				GM.UI.matchInfoMenu.GetPlayerHUD ( this ).UpdateStatusEffects ( InstanceID, Status );
			}
		}
		...
		protected override bool CommandAvailabilityCheck ( AbilityInstanceData ability, MoveData prerequisite )
		{
			...
			if ( !InstanceData.Ability2.IsPerkEnabled && Status.HasStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.EXHAUSTION ) )
				return false;
		...
		protected override void UseSpecial ( MoveData data )
		{
			...
			Tween t = transform.DOMove ( data.Destination.transform.position, MOVE_ANIMATION_TIME * 3 )
				...
				// Add status effect
				Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.EXHAUSTION, InstanceData.Ability1.Duration, this );
					GM.UI.matchInfoMenu.GetPlayerHUD ( this ).UpdateStatusEffects ( InstanceID, Status );
		...
		private void EndGrapple ( )
		{
			...
			Tween t2 = grappleTarget.transform.DOMove ( Vector3.Lerp ( transform.position, grappleTarget.transform.position, 0.75f ), GRAPPLE_ANIMATION_TIME )
				...
				grappleTarget.Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.GRAPPLED, this );
					GM.UI.matchInfoMenu.GetPlayerHUD ( grappleTarget ).UpdateStatusEffects ( grappleTarget.InstanceID, grappleTarget.Status );
		...
		private void EndGrappleDelegate ( Unit unit )
		{
			...
			// Remove the hero's status effect
			Status.RemoveStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.GRAPPLED, this );
			GM.UI.matchInfoMenu.GetPlayerHUD ( this ).UpdateStatusEffects ( InstanceID, Status );
		}
