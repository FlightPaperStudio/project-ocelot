...
namespace ProjectOcelot.Units
{
	public class Baetylus : Leader
	{
		...
		public override void ExecuteCommand ( )
		{
			...
			meteor.color = Tools.Util.TeamColor ( Owner.Team );
		...
		private List<Tween> ApplyMeteorSmash ( Hex hex )
		{
			...
			for ( int i = 0; i < hex.Neighbors.Length; i++ )
			{
				...
				if ( hex.Neighbors [ i ].Tile.CurrentUnit != null )
				{
					...
					// Apply status effect
					unit.Status.AddStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.STUNNED, InstanceData.Ability1.Duration, this );
		...
		private Hex GetPushBackHex ( Hex current, int direction, Hex destination )
		{
			...
			if ( current.Tile.CurrentUnit != null && current.Tile.CurrentUnit.Status.HasStatusEffect ( Database.StatusEffectDatabase.StatusEffectType.INCORPOREAL ) )
