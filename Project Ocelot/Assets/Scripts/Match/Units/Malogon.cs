...
public class Malogon : Leader
{
		...
		for ( int distance = 1; distance <= InstanceData.Ability1.PerkValue; distance++ )
		{
			...
			for ( int i = 0; i < targets.Count; i++ )
			{
				...
				if ( targets [ i ].Distance ( CurrentHex ) == distance )
				{
					...
					energy.color = Util.AccentColor ( Owner.Team );
	...
	private void ApplyPoison ( Unit unit )
	{
		...
		// Apply poison
		unit.Status.AddStatusEffect ( StatusEffectDatabase.StatusEffectType.RADIATION_POISONING, InstanceData.Ability1.Duration, this );
	}
