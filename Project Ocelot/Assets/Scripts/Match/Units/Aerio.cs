...
public class Aerio : Leader
{
	...
	public override void ExecuteCommand ( )
	{
		...
		for ( int i = 0; i < tiles.Count; i++ )
		{
			...
			Util.OrientSpriteToDirection ( gas, Owner.TeamDirection );
			...
			{
				// Apply status affect
				tiles [ i ].Tile.CurrentUnit.Status.AddStatusEffect ( StatusEffectDatabase.StatusEffectType.INCORPOREAL, InstanceData.Ability1.Duration, this );
	...
	protected override void OnDurationComplete ( AbilityInstanceData ability )
	{
		...
		for ( int i = 0; i < sublimationTargets.Count; i++ )
			if ( sublimationTargets [ i ] != null )
				sublimationTargets [ i ].Status.RemoveStatusEffect ( StatusEffectDatabase.StatusEffectType.INCORPOREAL, this );
