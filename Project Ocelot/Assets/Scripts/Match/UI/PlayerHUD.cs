...
namespace ProjectOcelot.Match.HUD
{
	public class PlayerHUD : MonoBehaviour
	{
		...
		public void Initialize ( Player player )
		{
			...
			playerName.color = Tools.Util.TeamColor ( Player.Team );
		...
		public void UpdatePortrait ( Units.Unit unit )
		{
		...
		public void AddPortrait ( Units.Unit newUnit, int adjacentID )
		{
		...
		public void UpdateStatusEffects ( int id, Units.StatusEffects status )
		{
			...
			for ( int i = 0; i < instanceHUDs [ id ].StatusIcons.Length; i++ )
			{
				...
				if ( i < status.Effects.Count )
				{
					...
					instanceHUDs [ id ].StatusIcons [ i ].color = Tools.Util.TeamColor ( status.Effects [ i ].Caster.Owner.Team );
				}
				else
				{
					// Hide icon
