	...
	public class TurnTimer : MonoBehaviour
	{
		...
		public void StopTimer ( )
		{
			...
			if ( GM.IsStartOfTurn )
			{
				...
				List<Units.Unit> units = GM.CurrentPlayer.UnitInstances.FindAll ( x => x.MoveList.Count > 0 );

				// Select a random unit
				int unitIndex = Random.Range ( 0, units.Count );
				Units.Unit unit = units [ unitIndex ];

				// Create list of possible moves
				List<Arena.MoveData> moves = unit.MoveList.FindAll ( x => x.PriorMove == null );

				// Select a random move
				int moveIndex = Random.Range ( 0, moves.Count );
				Arena.MoveData move = unit.MoveList [ moveIndex ];
		...
		private IEnumerator PanicMove ( Units.Unit unit, Arena.Hex hex )
		{
