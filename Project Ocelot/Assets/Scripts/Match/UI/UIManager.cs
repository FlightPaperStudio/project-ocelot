...
namespace ProjectOcelot.Match
{
	public class UIManager : MonoBehaviour
	{
		...
		public HUD.PathDrawer Path;
		public HUD.HexCursor Cursor;

		#endregion // UI Elements

		#region UI Data

		public Menues.MatchInfoMenu matchInfoMenu;
		public HUD.UnitHUD UnitHUD;
		...
		private void Update ( )
		{
			...
			if ( Input.GetKeyDown ( KeyCode.Escape ) && !popUp.IsOpen && !matchInfoMenu.IsOpen )
			{
				...
				if ( isPaused )
				{
					...
					foreach ( Menues.Menu m in pauseMenus )
					{
						if ( m.IsOpen )
						{
		...
		private IEnumerator WinCoroutine ( Player p )
		{
			...
			yield return splash.Shake ( p.PlayerName + " Wins!", Tools.Util.TeamColor ( p.Team ), false ).WaitForCompletion ( );
